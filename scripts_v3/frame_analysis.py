...
def _process_and_save_frame(
    ...
    fps: Optional[float] = None,    frame_idx_rel: Optional[int] = None,
    ...
    if utils.is_fade_frame(img):
        ...
        try:
            ...
            if success:
                logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: fade frame detected - saved to {skip_path.name} - preserving previous state")
            else:
                logging.error(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: fade frame detected but failed to save to {skip_path}")
        except Exception as e:
            logging.error(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: fade frame detected but error saving: {e}")
    ...
    try:
        text_res, _, bbox_res, ocr_error = utils.run_ocr(
            img, ocr_reader, ocr_engine_type,
    ...
    if ocr_error:
        ...
        logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) OCR SUCCESS: '{text_res or 'EMPTY'}' (length: {len(text_res or '')})")
    ...
    if prev_text:
        ...
        logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) TEXT COMPARISON: '{norm_text}' vs '{prev_text}'")
    ...
    if processing_mode == "static" and current_hash is not None and prev_hash is not None:
        ...
        logging.info(f"[{episode_id}] Frame {frame_num} (static) HASH DIFF: {hash_diff} (threshold: {config.HASH_DIFFERENCE_THRESHOLD})")
        ...
        if hash_diff < config.HASH_DIFFERENCE_THRESHOLD:
            ...
            logging.info(f"[{episode_id}] Frame {frame_num} (static) SKIPPED: similar hash {hash_diff} < {config.HASH_DIFFERENCE_THRESHOLD} - preserving previous state")
            return None, prev_text, prev_hash, prev_bbox
            ...
            skip_path = skipped_frames_dir / f"{prefix}static_skipped_{skip_reason}_num{frame_num:05d}.jpg"
            cv2.imwrite(str(skip_path), img)
            logging.info(f"[{episode_id}] Frame {frame_num} (static) SKIPPED: visual diff but no text - preserving previous state")
            return None, prev_text, prev_hash, prev_bbox
    ...
    if not has_sufficient_text:
        ...
        skip_path = skipped_frames_dir / f"{prefix}{mode_tag}skipped_{skip_reason}_num{frame_num:05d}.jpg"
        cv2.imwrite(str(skip_path), img)
        logging.warning(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: insufficient text length {len(norm_text)} < {config.MIN_OCR_TEXT_LENGTH} - preserving previous state")
        return None, prev_text, prev_hash, prev_bbox
    ...
    if episode_saved_texts_cache:        # Compare with the most recently saved text (last item in cache)
        ...
        if text_similarity > dynamic_threshold:
            ...
            if current_length > last_saved_length:
                ...
                logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) DECISION: SAVE - replacing shorter text ({last_saved_length} chars) with longer text ({current_length} chars)")
                logging.info(f"[{episode_id}] Removed shorter text from episode cache. Cache size: {len(episode_saved_texts_cache)}")
            else:
                # Last saved text is longer or equal - skip current
                save_frame = False
                skip_reason = f"similar_to_last_saved_shorter_{text_similarity}"
                logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) DECISION: SKIP - current text ({current_length} chars) is shorter than or equal to last saved ({last_saved_length} chars)")
        else:
            logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) DECISION: SAVE - sufficiently different from last saved frame")
    else:
        logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) EPISODE-LEVEL SIMILARITY: No previous saved text - similarity: {text_similarity}% (first frame)")
    ...
    if not save_frame:
        ...
        try:
            success = cv2.imwrite(str(skip_path), img)
            if success:
                logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: {skip_reason} - saved to {skip_path.name}")
            else:
                logging.error(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: {skip_reason} but failed to save to {skip_path}")
        except Exception as e:
            logging.error(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: {skip_reason} but error saving: {e}")
    ...
    if save_frame:
        ...
        if processing_mode == "dynamic":
            out_path = frames_output_dir / f"{prefix}scroll_{frame_count:03d}_num{frame_num:05d}_sim{int(text_similarity)}.jpg"
        elif processing_mode == "static":
            out_path = frames_output_dir / f"{prefix}static_{frame_count:03d}_num{frame_num:05d}_sim{int(text_similarity)}.jpg"
        else:  # single
        ...
        try:
            ...
            if success:
                ...
                logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SAVED and ADDED TO EPISODE CACHE: '{norm_text[:50]}...' (cache size: {len(episode_saved_texts_cache)})")
                ...
                if episode_saved_texts_cache is not None:
                    episode_saved_texts_cache.append(norm_text)
                    logging.info(f"[{episode_id}] Added text to episode cache. Cache size: {len(episode_saved_texts_cache)}")
                ...
                if episode_saved_files_cache is not None:
                    ...
                    logging.info(f"[{episode_id}] Added file path to episode files cache. Files cache size: {len(episode_saved_files_cache)}")
    ...
    try:
        success = cv2.imwrite(str(skip_path), img)
        if success:
            logging.info(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: save failed - saved to {skip_path.name} - preserving previous state")
        else:
            logging.error(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: save failed and couldn't save to skipped either")
    except Exception as e:
        logging.error(f"[{episode_id}] Frame {frame_num} ({processing_mode}) SKIPPED: save failed and error saving to skipped: {e}")
...
def analyze_candidate_scene_frames(
    ...
    global_last_saved_frame_hash_input: Optional[imagehash.ImageHash],    global_last_saved_ocr_bbox_input: Optional[Tuple[int, int, int, int]],
    # NEW: Episode-level saved text cache
    episode_saved_texts_cache: Optional[List[str]] = None,
    # NEW: File-to-cache mapping for physical file cleanup
    episode_saved_files_cache: Optional[List[str]] = None
    ...
    logging.info(f"[{episode_id}] Step 2: Analyzing frames for scene {scene_index} ({scene_info['start_frame']}-{scene_info['end_frame']}, Position: {scene_info.get('position', 'unknown')})...")

    current_scene_position = scene_info.get('position', 'unknown')

    analysis_info = {"type": "unknown", "output_files": [], "frame_count": 0, "median_flow_px_per_frame": 0.0, "status": "pending", "position": current_scene_position}
    ...
    try:
        ...
        pbar_read = tqdm(total=scene_info['end_frame'] - scene_info['start_frame'], desc=f"Read scene {scene_index}", unit="frame", leave=False)
        current_frame_num = scene_info['start_frame']
        while current_frame_num < scene_info['end_frame']:
            frame_img = video.read(decode=True)
            if frame_img is False: break
            ...
            if frame_img is None or not isinstance(frame_img, np.ndarray) or frame_img.size == 0:
                ...
                pbar_read.n = min(current_frame_num - scene_info['start_frame'], scene_info['end_frame'] - scene_info['start_frame'])
                pbar_read.refresh()
                continue
        ...
        if scene_info.get('position', 'unknown') == "second_half":
            ...
            try:
                ...
                vertical_flows = [
                    utils.calculate_vertical_flow(g1, g2)
                    for g1, g2 in zip(gray_frames, gray_frames[1:])
                ]
            ...
            analysis_info["type"] = "static_first_half" 
             ...
             logging.warning(f"[{episode_id}] Scene {scene_index} has unknown position '{scene_info.get('position', 'unknown')}'. Defaulting to static/slow logic.")
        ...
        if apply_dynamic_scroll_logic:
            ...
            for i in tqdm(range(0, num_frames, frames_per_save), desc=f"Select scroll frames scene {scene_index}", unit="frame", leave=False):
                logging.info("-" * 80) 
                ...
                logging.info(f"[{episode_id}] Scene {scene_index} Frame {frame_num} (Dynamic Scroll Sample) Processing...")
                ...
                metadata, new_text, new_hash, new_bbox = _process_and_save_frame(
                    ...
                    scene_position=current_scene_position,
                    prev_bbox=current_comparator_bbox,                    frame_width=frame_width,
            ...
            if scene_duration_seconds > config.HASH_SAMPLE_INTERVAL_SECONDS and num_frames > 1 and fps > 0:
                ...
                pbar_hash = tqdm(sampled_indices_rel, desc=f"Hash/OCR(sq) scene {scene_index}", unit="sample", leave=False)
                ...
                for frame_idx_rel in pbar_hash:
                    ...
                    metadata, new_text, new_hash, new_bbox = _process_and_save_frame(
                        ...
                        fps=fps,                        frame_idx_rel=frame_idx_rel,
                        episode_saved_texts_cache=episode_saved_texts_cache,  # Pass episode cache
                        episode_saved_files_cache=episode_saved_files_cache   # Pass episode files cache
                    )
