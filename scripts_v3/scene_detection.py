...
def _get_segments_to_process(
    total_frames: int,
    fps: float,
    custom_time_segments: Optional[List[Tuple[str, str]]]
...
def _filter_scenes_by_count(scenes: List[Tuple[FrameTimecode, FrameTimecode, int]], 
                           start_count: int, end_count: int) -> List[Tuple[FrameTimecode, FrameTimecode, int]]:
    """
...
def identify_candidate_scenes(
    ...
    try:
        ...
        if scenes_cache_file.exists():
            try:
                with open(scenes_cache_file, 'r', encoding='utf-8') as f:
                    ...
                    if (cached_data.get('total_frames') == total_frames and 
                        cached_data.get('fps') == fps):
        ...
        if scenes_in_video is None:
            ...
            scene_manager.add_detector(ThresholdDetector(threshold=config.SceneDetectionConfig.THRESHOLD, fade_bias=0.5))
        ...
        if status != SceneDetectionStatus.COMPLETED_NO_SCENES_IN_SEGMENTS:  # Only proceed if scenes were found
            ...
            log.info(f"Found {len(valid_scenes_for_ocr_with_tags)} scenes after min length filter ({config.SCENE_MIN_LENGTH_FRAMES} frames). Processing these for OCR.")
            ...
            if not valid_scenes_for_ocr_with_tags:
                ...
                for scene_idx, (start_time, end_time, temporal_tag_for_scene, origin_tag_for_scene) in enumerate(tqdm(valid_scenes_for_ocr_with_tags, desc=f"[{episode_id}] OCR Sampling & Analysis", unit="scene")):
                    current_scene_data = {
                    ...
                    for sample_idx, sample_point_ratio in enumerate(config.INITIAL_FRAME_SAMPLE_POINTS):
                        ...
                        if frame_img_np is not None:
                            ...
                            image_context_id = f"{episode_id}_scene{scene_idx}_sample{sample_idx}_frame{target_frame_num}"
                            ...
                            while ocr_attempt_count < MAX_OCR_ATTEMPTS and not ocr_success:
                                try:
                                    ...
                                    if error_ocr:
                                        logging.warning(f"[{episode_id}] OCR error for scene {scene_idx}, frame {target_frame_num} (attempt {ocr_attempt_count + 1}): {error_ocr}")
                                        current_scene_data["ocr_errors_in_scene"].append(f"Frame {target_frame_num}: {error_ocr}")
                                    ocr_success = True # Assume success if no exception, error string will be handled
                                except Exception as ocr_exc:
                                    logging.error(f"[{episode_id}] Exception during OCR for scene {scene_idx}, frame {target_frame_num}, attempt {ocr_attempt_count + 1}: {ocr_exc}", exc_info=False)
                                    error_ocr = str(ocr_exc)
                                    current_scene_data["ocr_errors_in_scene"].append(f"Frame {target_frame_num} Exception: {error_ocr}")
                            ...
                            if text and not error_ocr:
                                ...
                                if cleaned_text_for_norm != text:
                                    logging.debug(f"[{episode_id}] Scene {scene_idx} SampleFrame {target_frame_num}: Stripped OCR raw text from {repr(text)} to {repr(cleaned_text_for_norm)}")
                                
                                normalized_text_for_check = utils.normalize_text_for_comparison(cleaned_text_for_norm, user_stopwords)
                                
                                logging.debug(f"[{episode_id}] Scene {scene_idx} SampleFrame {target_frame_num}: OCR_Raw={repr(text)}, CleanedForNorm={repr(cleaned_text_for_norm)}, Normalized={repr(normalized_text_for_check)}, LenNormalized={len(normalized_text_for_check)}, MinLength={config.MIN_OCR_TEXT_LENGTH}, PassesCheck={(len(normalized_text_for_check) >= config.MIN_OCR_TEXT_LENGTH)})")
                                ...
                                if len(normalized_text_for_check) >= config.MIN_OCR_TEXT_LENGTH:
                                    ...
                                    try:
                                        ...
                                        logging.error(f"[{episode_id}] Error saving representative frame {target_frame_num} for scene {scene_idx}: {e_save_repr}")
                    ...
                    if current_scene_data["has_potential_text"]:
                        ...
                        logging.debug(f"[{episode_id}] Scene {scene_idx} (Frames {start_time.get_frames()}-{end_time.get_frames()}, {temporal_tag_for_scene}/{origin_tag_for_scene}) did not yield potential text after sampling.")
        ...
        log.info(f"Step 1 processing. Final Status: {status.value}. Found {len(candidate_scenes_data)} candidate scenes.")
